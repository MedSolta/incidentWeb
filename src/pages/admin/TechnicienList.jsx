...
const TechnicienList = () => {
    ...
    useEffect(() => {
        const fetchTechniciens = async () => {
            try {
                setLoading(true);
                const endpoint = filterStatus === 'actif' ? '/techniciens' : '/techniciens/technicien-archiver';
    ...
    const handleSearch = (e) => {
        ...
        const filtered = techniciens.filter(tech =>
            String(tech.nom || '').toLowerCase().includes(term) ||
            String(tech.prenom || '').toLowerCase().includes(term) ||
            String(tech.email || '').toLowerCase().includes(term)
        );
    ...
    const handleDelete = async (id) => {
        try {
            await api.put(`/techniciens/archiver/${id}`);
            setTechniciens(techniciens.filter(tech => tech.id !== id));
            setFilteredTechniciens(filteredTechniciens.filter(tech => tech.id !== id));
        } catch (err) {
    ...
    const handleSendMessage = (technicien) => {
    ...
    const handleMessageSubmit = async () => {
        try {
            await api.post('/admins/send-message', {
                recipient_id: selectedTechnicien.id,
    ...
    const columns = [
        ...
        { key: 'code_postal', label: 'Code Postal' },
        {
            ...
            label: 'Disponibilité',
            render: (item) => (
                <span className={`badge ${item.disponibilite ? 'bg-success' : 'bg-secondary'}`}>
        ...
        },
        {
            ...
            label: 'Statut',
            render: (item) => (
                <span className={`badge ${item.acceptation ? 'bg-primary' : 'bg-warning'}`}>
                    {item.acceptation ? 'Accepté' : 'En attente'}
                </span>
            )
        }
    ];
            ...
            <div className="card shadow-sm">
                <div className="card-body">
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <Link to="/admin/technicien-add" className="btn btn-primary">
                    ...
                    <DataTable
                        ...
                        columns={columns}
                        onView={(id) => navigate(`/admin/technicien-detail/${id}`)}
                        onEdit={(id) => navigate(`/admin/technicien-edit/${id}`)}
                        onDelete={(item) => {
                    ...
                    <DeleteModal
                        ...
                        onConfirm={() => handleDelete(selectedTechnicien?.id)}
                        itemName={`le technicien ${selectedTechnicien?.prenom} ${selectedTechnicien?.nom}`}
                    />
                                ...
                                <div className="modal-content">
                                    <div className="modal-header">
                                        <h5 className="modal-title">
                                            Envoyer un message à {selectedTechnicien?.prenom} {selectedTechnicien?.nom}
                                        </h5>
                                        <button
                                            type="button"
                                            className="btn-close"
